...
1. **`.opencode/component_catalog.md` (and `.opencode/references/component_catalog_full.md`)** — the visual catalog. This is
...
3. The matching page skill(s) under `.opencode/skills/` (e.g. `.opencode/skills/create-list-page/SKILL.md`) for the final code blueprint.
...
Write the decomposition to `.opencode/temp/<ScreenName>_visual_spec.json` as a component tree. Each node:
...
Then write a short human summary to `.opencode/temp/<ScreenName>_visual_analysis.md`:
