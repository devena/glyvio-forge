...
1. Determine the temp file path: `.opencode/temp/<ModalName>_design.json` (e.g., `CustomUserEditModal_design.json`).
...
4. Retrieve the design JSON using the browser inspection script:
   ...
   - Run the script `.opencode/scripts/chrome_inspector.js` via the available shell tool to connect to Chrome, automatically wait for `window.__finalDesign` to be populated, and save the retrieved JSON to `.opencode/temp/<ModalName>_design.json`.
     _Command:_ `node .opencode/scripts/chrome_inspector.js <ModalName>` (e.g., `node .opencode/scripts/chrome_inspector.js CustomUserEditModal`).
