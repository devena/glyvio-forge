...
1. Determine the temp file path: `.opencode/temp/<SidebarName>_design.json` (e.g., `TaskTypeSidebar_design.json`).
...
4. Retrieve the design JSON using the browser inspection script:
   ...
   - Run the script `.opencode/scripts/chrome_inspector.js` via the available shell tool to connect to Chrome, automatically wait for `window.__finalDesign` to be populated, and save the retrieved JSON to `.opencode/temp/<SidebarName>_design.json`.
     _Command:_ `node .opencode/scripts/chrome_inspector.js <SidebarName>` (e.g., `node .opencode/scripts/chrome_inspector.js TaskTypeSidebar`).
