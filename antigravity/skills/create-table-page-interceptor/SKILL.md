---
name: create-table-page-interceptor
description: "Generates a custom class extending an abstract `SimpleTablePageInterceptor` to dynamically modify table page design, row layouts, and database query filters, and registers it."
---
<!-- Generated from src/skills/create-table-page-interceptor/SKILL.md by tools/generate.py. Edit the source, not this file. -->
# Agent Skill: Create Table Page Interceptor in Glyvio

This document defines a structured AI agent skill. Other AI coding agents or developers can load and execute this skill to generate and register an interceptor/listener to customize the behavior of an existing `SimpleTablePage` view in another project.

---

## 🎯 Skill Metadata

- **Name**: `create_table_page_interceptor`
- **Description**: Generates a custom class extending an abstract `SimpleTablePageInterceptor` to dynamically modify table page design, row layouts, and database query filters, and registers it.
...
1. Determine the temp file path: `antigravity/temp/<TablePageName>_design.json` (e.g., `ProductTablePage_design.json`).
...
3. Create and register a temporary **SpyInterceptor** in the plugin:
   ...
   - Implement `SpyInterceptor` extending `<TargetBaseInterceptorClass>` to capture the `design` in `getDesign(state: <TargetStateClass>, design: glyvio_core.SimpleTablePageDesign)`:
     ```typescript
     export class SpyInterceptor extends <TargetBaseInterceptorClass> {
       ...
       override getDesign(state: <TargetStateClass>, design: glyvio_core.SimpleTablePageDesign): void {
...
4. Retrieve the design JSON using the browser inspection script:
   ...
   - Run the script `antigravity/scripts/chrome_inspector.js` via the available shell tool to connect to Chrome, automatically wait for `window.__finalDesign` to be populated, and save the retrieved JSON to `antigravity/temp/<TablePageName>_design.json`.
     _Command:_ `node antigravity/scripts/chrome_inspector.js <TablePageName>` (e.g., `node antigravity/scripts/chrome_inspector.js ProductTablePage`).
...
5. Once the design JSON is collected and saved to `antigravity/temp/<TablePageName>_design.json`, remove the temporary `SpyInterceptor` and its registration from the codebase, and run `pnpm build` again to clean up the compiled distribution.
6. If Chrome debugging is not available or fails, fallback to asking the user to send the **current design JSON** of the page (obtained from `page.getDesignRaw(state)` or the console) in the next message, or to save it manually to `antigravity/temp/<TablePageName>_design.json`.
...
After completing the Step 0.1 analysis, **immediately write the findings to `antigravity/temp/<TablePageName>_analysis.md`** using the available file-editing tool. Include:
...
> "This task requires modifying the table page row design. Please provide the current `TableLayoutRowDesign` JSON for a sample row. You can obtain it by temporarily adding a `console.log(JSON.stringify(design))` inside a `getDesignForRow` override, then copying the output from the browser console."

- Save the received JSON to `antigravity/temp/<TablePageName>_rowDesign.json`.
...
1. **Target Table Page Name** (e.g., `ProductTablePage`): The name of the Table Page view class being intercepted.
2. **Target Route Class** (e.g., `ProductTablePageRoute`): The route class name of the target page.
3. **Target Base Interceptor Class** (e.g., `ProductTablePageInterceptor`): The abstract base class of the interceptor defined by the target page.
...
4. **Target State Class** (e.g., `ProductTablePageState`): The state interface of the target page.
5. **Target Entity Name** (e.g., `Product`): The name of the model in `glyvio_entity.*` associated with the page.
6. **Listener Unique ID** (e.g., `custom_product_table_page_interceptor`): A unique identifier for the registered listener.
7. **Modifications Required**:
   - **Page Design Modifications**: New filter fields, App Bar buttons, or columns to add.
   - **Row Design Modifications**: Custom cells, icons, or text styling to inject into the rows.
...
glyvio_core.appInterceptorService.registerInterceptors([
  {
    interceptor: YourCustomTablePageInterceptor,
...
/**
 * Custom Interceptor to dynamically augment the behavior of <TargetTablePageName>.
 */
export class <InterceptorClassName> extends <TargetBaseInterceptorClass> {
  ...
  /**
   * Intercepts the SimpleTablePageDesign configuration to inject new filters, columns, or buttons.
  ...
  /**
   * Intercepts the row design to inject custom layout cells, options, or decorators.
   */
  override getDesignForRow(
    ...
    // 💡 Example: Adding a custom action cell to the table row:
  }
