---
name: create-calendar-page-interceptor
description: "Generates a custom class extending an abstract `SimpleCalendarPageInterceptor` to dynamically modify calendar designs, cell layouts, and database query filters, and registers it."
---
<!-- Generated from src/skills/create-calendar-page-interceptor/SKILL.md by tools/generate.py. Edit the source, not this file. -->
# Agent Skill: Create Calendar Page Interceptor in Glyvio

This document defines a structured AI agent skill. Other AI coding agents or developers can load and execute this skill to generate and register an interceptor/listener to customize the behavior of an existing `SimpleCalendarPage` view in another project.

---

## 🎯 Skill Metadata

- **Name**: `create_calendar_page_interceptor`
- **Description**: Generates a custom class extending an abstract `SimpleCalendarPageInterceptor` to dynamically modify calendar designs, cell layouts, and database query filters, and registers it.
...
1. Determine the temp file path: `antigravity/temp/<CalendarPageName>_design.json` (e.g., `TaskCalendarPage_design.json`).
...
3. Create and register a temporary **SpyInterceptor** in the plugin:
   ...
   - Implement `SpyInterceptor` extending `<TargetBaseInterceptorClass>` to capture the `design` in `getDesign(state: <TargetStateClass>, design: glyvio_core.SimpleCalendarPageDesign)`:
     ```typescript
     export class SpyInterceptor extends <TargetBaseInterceptorClass> {
       ...
       override getDesign(state: <TargetStateClass>, design: glyvio_core.SimpleCalendarPageDesign): void {
...
4. Retrieve the design JSON using the browser inspection script:
   ...
   - Run the script `antigravity/scripts/chrome_inspector.js` via the available shell tool to connect to Chrome, automatically wait for `window.__finalDesign` to be populated, and save the retrieved JSON to `antigravity/temp/<CalendarPageName>_design.json`.
     _Command:_ `node antigravity/scripts/chrome_inspector.js <CalendarPageName>` (e.g., `node antigravity/scripts/chrome_inspector.js TaskCalendarPage`).
...
5. Once the design JSON is collected and saved to `antigravity/temp/<CalendarPageName>_design.json`, remove the temporary `SpyInterceptor` and its registration from the codebase, and run `pnpm build` again to clean up the compiled distribution.
6. If Chrome debugging is not available or fails, fallback to asking the user to send the **current design JSON** of the page (obtained from `page.getDesignRaw(state)` or the console) in the next message, or to save it manually to `antigravity/temp/<CalendarPageName>_design.json`.
...
After completing the Step 0.1 analysis, **immediately write the findings to `antigravity/temp/<CalendarPageName>_analysis.md`** using the available file-editing tool. Include:
...
1. **Target Calendar Page Name** (e.g., `TaskCalendarPage`): The name of the Calendar Page view class being intercepted.
2. **Target Route Class** (e.g., `TaskCalendarPageRoute`): The route class name of the target page.
3. **Target Base Interceptor Class** (e.g., `TaskCalendarPageInterceptor`): The abstract base class of the interceptor defined by the target page.
...
4. **Target State Class** (e.g., `TaskCalendarPageState`): The state interface of the target page.
5. **Target Entity Name** (e.g., `Task`): The name of the model in `glyvio_entity.*` associated with the page.
6. **Listener Unique ID** (e.g., `custom_task_calendar_page_interceptor`): A unique identifier for the registered listener.
7. **Modifications Required**:
   - **Page Design Modifications**: New filters in sidebar, custom buttons, layout views allowed, or title overrides.
   - **Cell Modifications**: Custom descriptions, badges, layouts, colors, or action buttons inside the calendar cell.
...
glyvio_core.appInterceptorService.registerInterceptors([
  {
    interceptor: YourCustomCalendarPageInterceptor,
...
/**
 * Custom Interceptor to dynamically augment the behavior of <TargetCalendarPageName>.
 */
export class <InterceptorClassName> extends <TargetBaseInterceptorClass> {
  ...
  /**
   * Intercepts the SimpleCalendarPageDesign configuration to inject new filters or buttons.
  ...
  override getDesign(state: <TargetStateClass>, design: glyvio_core.SimpleCalendarPageDesign): void {
    ...
    // Example: Changing allowed layouts:
    // design.allowedViews = ['month', 'week'];
  }

  /**
   * Intercepts visual layout details before rendering each calendar event cell.
   */
  override getDesignForCell(
    state: <TargetStateClass>,
    item: glyvio_entity.<TargetEntityName>,
    design: glyvio_core.SimpleCalendarCellDesign,
  ): void {
    // 💡 Example: Highlight cell card if event has specific parameters
    // if (item.deleted) {
