---
name: create-entity-modal-interceptor
description: "Generates a custom class extending an abstract `SimpleEntityModalInterceptor` to dynamically modify entity modal search list cell layouts, selected chip formats, app bar designs, and lookup query filter constraints, and registers it."
---
<!-- Generated from src/skills/create-entity-modal-interceptor/SKILL.md by tools/generate.py. Edit the source, not this file. -->
# Agent Skill: Create Entity Modal Interceptor in Glyvio

This document defines a structured AI agent skill. Other AI coding agents or developers can load and execute this skill to generate and register an interceptor/listener to customize the behavior of an existing `SimpleEntityModal` view in another project.

---

## 🎯 Skill Metadata

- **Name**: `create_entity_modal_interceptor`
- **Description**: Generates a custom class extending an abstract `SimpleEntityModalInterceptor` to dynamically modify entity modal search list cell layouts, selected chip formats, app bar designs, and lookup query filter constraints, and registers it.
...
1. Determine the temp file path: `antigravity/temp/<ModalName>_design.json` (e.g., `ChatAgentEntityModal_design.json`).
...
3. Create and register a temporary **SpyInterceptor** in the plugin:
   ...
   - Implement `SpyInterceptor` extending `<TargetBaseInterceptorClass>` to capture the `design` in `getDesign(state: <TargetStateClass>, design: glyvio_core.SimpleEntityModalDesign)`:
     ```typescript
     export class SpyInterceptor extends <TargetBaseInterceptorClass> {
       ...
       override getDesign(state: <TargetStateClass>, design: glyvio_core.SimpleEntityModalDesign): void {
...
4. Retrieve the design JSON using the browser inspection script:
   ...
   - Run the script `antigravity/scripts/chrome_inspector.js` via the available shell tool to connect to Chrome, automatically wait for `window.__finalDesign` to be populated, and save the retrieved JSON to `antigravity/temp/<ModalName>_design.json`.
     _Command:_ `node antigravity/scripts/chrome_inspector.js <ModalName>` (e.g., `node antigravity/scripts/chrome_inspector.js ChatAgentEntityModal`).
...
5. Once the design JSON is collected and saved to `antigravity/temp/<ModalName>_design.json`, remove the temporary `SpyInterceptor` and its registration from the codebase, and run `pnpm build` again to clean up the compiled distribution.
6. If Chrome debugging is not available or fails, fallback to asking the user to send the **current design JSON** of the modal (obtained from `modal.getDesignRaw(state)` or the console) in the next message, or to save it manually to `antigravity/temp/<ModalName>_design.json`.
...
After completing the Step 0.1 analysis, **immediately write the findings to `antigravity/temp/<ModalName>_analysis.md`** using the available file-editing tool. Include:
...
1. **Target Entity Modal Name** (e.g., `ChatAgentEntityModal`): The name of the Entity Modal view class being intercepted.
2. **Target Route Class** (e.g., `ChatAgentEntityModalRoute`): The route class name of the target modal.
3. **Target Base Interceptor Class** (e.g., `ChatAgentEntityModalInterceptor`): The abstract base class of the interceptor defined by the target modal.
...
4. **Target State Class** (e.g., `ChatAgentEntityModalState`): The state interface of the target modal.
5. **Target Entity Name** (e.g., `ChatAgent`): The name of the model in `glyvio_entity.*` associated with the modal.
6. **Listener Unique ID** (e.g., `custom_chat_agent_entity_modal_interceptor`): A unique identifier for the registered listener.
7. **Modifications Required**:
   - **Modal Design Modifications**: Change titles, App Bar options, or layout structure.
   - **Cell Layout Modifications**: Customize fields displayed inside list view cards.
   - **Chip Design Modifications**: Customize text styling or icons inside selected chips.
   - **Query Modifications**: Enforce additional database query filters or select joins.
   - **Search Input Modifications**: Intercept text query match criteria.
...
glyvio_core.appInterceptorService.registerInterceptors([
  {
    interceptor: YourCustomEntityModalInterceptor,
...
/**
 * Custom Interceptor to dynamically augment the behavior of <TargetEntityModalName>.
 */
export class <InterceptorClassName> extends <TargetBaseInterceptorClass> {
  ...
  /**
   * Intercepts the SimpleEntityModalDesign configuration to customize visual layouts or App Bars.
  ...
  override getDesign(state: <TargetStateClass>, design: glyvio_core.SimpleEntityModalDesign): void {
    ...
    // if (appBar) {
    //   appBar.title = `${appBar.title} (Custom Selection)`;
    // }
  }

  /**
   * Intercepts and alters cell layout designs representing rows inside search list.
   */
  override getDesignForCell(
    state: <TargetStateClass>,
    item: glyvio_entity.<TargetEntityName>,
    design: glyvio_core.LineCellDesign,
  ): void {
    // 💡 Example: Add custom decoration/text to list cell card
  }

  /**
   * Intercepts and alters chip design layout representing selected item links.
   */
  override getDesignForChip(
    item: glyvio_entity.<TargetEntityName>,
    design: glyvio_core.ChipDesign,
  ): void {
    // 💡 Example: Change chip color theme or icons
    // design.colorTheme = 'GREEN';
  }

  /**
   * Intercepts select strings returned in autocomplete textfields.
   */
  override getDesignForSelectText(
    state: <TargetStateClass>,
    item: glyvio_entity.<TargetEntityName>,
  ...
  /**
   * Intercepts visual components displayed inside dropdown selector cards.
   */
  override getDesignForSelectChild(
    state: <TargetStateClass>,
    item: glyvio_entity.<TargetEntityName>,
    design: glyvio_core.WidgetDesign,
  ): glyvio_core.WidgetDesign {
    return design;
  }

  /**
   * Hook to modify or add query builders filters when loading selection lists.
   */
  override populateQueryBuilder(
    state: <TargetStateClass>,
    queryBuilder: glyvio_core.QueryBuilder<glyvio_entity.<TargetEntityName>>,
  ): void {
    // 💡 Example: Enforce dynamic visibility filter
  }

  /**
   * Hook to intercept autocomplete search text filter mappings.
   */
  override populateAutocompleteFilter(
    state: <TargetStateClass>,
    filter: glyvio_core.QueryBuilderFilter,
    text: string,
  ): void {
    // 💡 Example: Add extra OR clause options to keyword matching
  }

  /**
   * Hook to intercept item select text query filters.
   */
  override populateSelectFilter(
    state: <TargetStateClass>,
    filter: glyvio_core.QueryBuilderFilter,
    text: string,
  ): void {
    // 💡 Example: Match select text filters precisely
  }
