---
name: create-send-modal-interceptor
description: "Generates a custom class extending an abstract `SimpleSendModalInterceptor` to dynamically modify message send modal layouts, recipient contact persons, compiled files/attachments, or lifecycle triggers prior to sending."
---
<!-- Generated from src/skills/create-send-modal-interceptor/SKILL.md by tools/generate.py. Edit the source, not this file. -->
# Agent Skill: Create SimpleSendModal Interceptor in Glyvio

This document defines a structured AI agent skill. Other AI coding agents or developers can load and execute this skill to generate and register an interceptor/listener to customize the behavior of an existing `SimpleSendModal` view in another project.

---

## 🎯 Skill Metadata

- **Name**: `create_send_modal_interceptor`
- **Description**: Generates a custom class extending an abstract `SimpleSendModalInterceptor` to dynamically modify message send modal layouts, recipient contact persons, compiled files/attachments, or lifecycle triggers prior to sending.
...
1. Determine the temp file path: `antigravity/temp/<ModalName>_design.json` (e.g., `InvoiceSendModal_design.json`).
...
3. Create and register a temporary **SpyInterceptor** in the plugin:
   ...
   - Implement `SpyInterceptor` extending `<TargetBaseInterceptorClass>` to capture the `design` in `getDesign(state: <TargetStateClass>, design: glyvio_core.SimpleSendModalDesign)`:
     ```typescript
     export class SpyInterceptor extends <TargetBaseInterceptorClass> {
       ...
       override getDesign(state: <TargetStateClass>, design: glyvio_core.SimpleSendModalDesign): void {
...
4. Retrieve the design JSON using the browser inspection script:
   ...
   - Run the script `antigravity/scripts/chrome_inspector.js` via the available shell tool to connect to Chrome, automatically wait for `window.__finalDesign` to be populated, and save the retrieved JSON to `antigravity/temp/<ModalName>_design.json`.
     _Command:_ `node antigravity/scripts/chrome_inspector.js <ModalName>` (e.g., `node antigravity/scripts/chrome_inspector.js InvoiceSendModal`).
...
1. **Target Send Modal Name** (e.g., `InvoiceSendModal`): The name of the Send Modal view class being intercepted.
2. **Target Route Class** (e.g., `InvoiceSendModalRoute`): The route class name of the target modal.
3. **Target Base Interceptor Class** (e.g., `InvoiceSendModalInterceptor`): The abstract base class of the interceptor defined by the target modal.
...
4. **Target State Class** (e.g., `InvoiceSendModalState`): The state interface of the target modal.
5. **Listener Unique ID** (e.g., `custom_invoice_send_modal_interceptor`): A unique identifier for the registered listener.
6. **Modifications Required**:
   - **Modal Design Modifications**: Customize titles, App Bar options, or layout structure.
   - **Recipient modifications**: Filter, append, or modify the loaded `SimpleSendModalPersonDTO[]` list of recipient contacts.
   - **Attachments modifications**: Intercept and alter compiled attachment/report list files before they are sent.
   - **On Send Trigger**: Handle custom validation or trigger background processes when the user taps send.
...
glyvio_core.appInterceptorService.registerInterceptors([
  {
    interceptor: YourCustomSendModalInterceptor,
...
/**
 * Custom Interceptor to dynamically augment the behavior of <TargetSendModalName>.
 */
export class <InterceptorClassName> extends <TargetBaseInterceptorClass> {
  ...
  /**
   * Intercepts the SimpleSendModalDesign configuration to customize visual layouts or App Bars.
  ...
  override getDesign(state: <TargetStateClass>, design: glyvio_core.SimpleSendModalDesign): void {
    ...
    // Example: Change the modal App Bar title
  ...
  /**
   * Intercepts and modifies the loaded recipient contact persons list.
   */
  override async getPersonsDTO(state: <TargetStateClass>, persons: glyvio_core.SimpleSendModalPersonDTO[]): Promise<void> {
    // 💡 Example: Append an additional CC contact to each recipient person
  }

  /**
   * Intercepts and updates compiled lists of attachments before sending.
   */
  override async getFilesProcessed(state: <TargetStateClass>, files: glyvio_core.PluginServiceInterfaceChatAgentAttachment[]): Promise<void> {
    // 💡 Example: Inspect or inject extra PDF documents in the attachment list
  }

  /**
   * Lifecycle event hook triggered prior to transmission.
   */
  override async onSend(state: <TargetStateClass>): Promise<void> {
    // 💡 Example: Perform custom state validations before dispatching messages
  }
