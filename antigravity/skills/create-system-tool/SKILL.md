...
4. **Return a string for the LLM**: `handle` returns a `string`. Design it to be highly readable by an LLM, and append explicit `Instructions for the LLM: ...` at the end telling it how to present the data to the user.
   - If that presented text (or a JSON field within it, e.g. an alert's `description`) will be rendered to the end user as markdown inside `glyvio_app`, apply `antigravity/skills/format-llm-markdown-output/SKILL.md` — do not let the LLM invent HTML or unsupported markdown syntax.
