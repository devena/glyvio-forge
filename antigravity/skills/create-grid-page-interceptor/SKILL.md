---
name: create-grid-page-interceptor
description: "Generates a custom class extending an abstract `SimpleGridPageInterceptor` to dynamically modify grid/gallery page designs, cell card layouts, and database query filters, and registers it."
---
<!-- Generated from src/skills/create-grid-page-interceptor/SKILL.md by tools/generate.py. Edit the source, not this file. -->
# Agent Skill: Create Grid Page Interceptor in Glyvio

This document defines a structured AI agent skill. Other AI coding agents or developers can load and execute this skill to generate and register an interceptor/listener to customize the behavior of an existing `SimpleGridPage` view in another project.

---

## 🎯 Skill Metadata

- **Name**: `create_grid_page_interceptor`
- **Description**: Generates a custom class extending an abstract `SimpleGridPageInterceptor` to dynamically modify grid/gallery page designs, cell card layouts, and database query filters, and registers it.
...
1. Determine the temp file path: `antigravity/temp/<GridPageName>_design.json` (e.g., `TaskTypeGridPage_design.json`).
...
3. Create and register a temporary **SpyInterceptor** in the plugin:
   ...
   - Implement `SpyInterceptor` extending `<TargetBaseInterceptorClass>` to capture the `design` in `getDesign(state: <TargetStateClass>, design: glyvio_core.SimpleGridPageDesign)`:
     ```typescript
     export class SpyInterceptor extends <TargetBaseInterceptorClass> {
       ...
       override getDesign(state: <TargetStateClass>, design: glyvio_core.SimpleGridPageDesign): void {
...
4. Retrieve the design JSON using the browser inspection script:
   ...
   - Run the script `antigravity/scripts/chrome_inspector.js` via the available shell tool to connect to Chrome, automatically wait for `window.__finalDesign` to be populated, and save the retrieved JSON to `antigravity/temp/<GridPageName>_design.json`.
     _Command:_ `node antigravity/scripts/chrome_inspector.js <GridPageName>` (e.g., `node antigravity/scripts/chrome_inspector.js TaskTypeGridPage`).
...
5. Once the design JSON is collected and saved to `antigravity/temp/<GridPageName>_design.json`, remove the temporary `SpyInterceptor` and its registration from the codebase, and run `pnpm build` again to clean up the compiled distribution.
6. If Chrome debugging is not available or fails, fallback to asking the user to send the **current design JSON** of the page (obtained from `page.getDesignRaw(state)` or the console) in the next message, or to save it manually to `antigravity/temp/<GridPageName>_design.json`.
...
After completing the Step 0.1 analysis, **immediately write the findings to `antigravity/temp/<GridPageName>_analysis.md`** using the available file-editing tool. Include:
...
If the task requires modifications to the card cell layout (i.e., implementing or changing `getDesignForCell`), **before writing any code**, ask the user for the current **card cell design JSON**.

Prompt the user with a message like:

> "This task requires modifying the grid card cell design. Please provide the current `CardCellDesign` JSON for a sample card. You can obtain it by temporarily adding a `console.log(JSON.stringify(design))` inside a `getDesignForCell` override, then copying the output from the browser console."

- Save the received JSON to `antigravity/temp/<GridPageName>_cellDesign.json`.
...
1. **Target Grid Page Name** (e.g., `TaskTypeGridPage`): The name of the Grid Page view class being intercepted.
2. **Target Route Class** (e.g., `TaskTypeGridPageRoute`): The route class name of the target page.
3. **Target Base Interceptor Class** (e.g., `TaskTypeGridPageInterceptor`): The abstract base class of the interceptor defined by the target page.
...
4. **Target State Class** (e.g., `TaskTypeGridPageState`): The state interface of the target page.
5. **Target Entity Name** (e.g., `TaskType`): The name of the model in `glyvio_entity.*` associated with the page.
6. **Listener Unique ID** (e.g., `custom_task_type_grid_page_interceptor`): A unique identifier for the registered listener.
7. **Modifications Required**:
   - **Page Design Modifications**: New filters in sidebar, custom buttons, cell height/width modifications, or title overrides.
   - **Cell Card Modifications**: Custom badges, layouts, colors, or action buttons inside the card cell.
...
glyvio_core.appInterceptorService.registerInterceptors([
  {
    interceptor: YourCustomGridPageInterceptor,
...
/**
 * Custom Interceptor to dynamically augment the behavior of <TargetGridPageName>.
 */
export class <InterceptorClassName> extends <TargetBaseInterceptorClass> {
  ...
  /**
   * Intercepts the SimpleGridPageDesign configuration to inject new filters or buttons.
  ...
  override getDesign(state: <TargetStateClass>, design: glyvio_core.SimpleGridPageDesign): void {
    ...
    // Example: Changing cell size overrides:
    // design.cellWidth = 300;
    // design.cellHeight = 120;
  }

  /**
   * Intercepts visual layout details before rendering each grid card cell.
   */
  override getDesignForCell(
    ...
    // 💡 Example: Highlight cell card if item has specific parameters
    // if (item.deleted) {
  ...
  /**
   * Hook to modify or add criteria to the query builder loading elements.
   */
  override populateQueryBuilder(
    state: <TargetStateClass>,
    queryBuilder: glyvio_core.QueryBuilder<glyvio_entity.<TargetEntityName>>,
  ): void {
    // 💡 Example: Filter out inactive records dynamically
  }
  ...
  override populateMainFilter(
    ...
    // 💡 Example: Add another OR clause to match custom fields in search
  }
