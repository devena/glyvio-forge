...
4. **System Save Operations**: When persisting supplementary records during validation, use `glyvio_core.entityService.saveEntityWithoutPermission(entity)` so the write executes successfully regardless of the current user's permission scope.
   - Every supplementary entity newly created here must have an explicit `userGroupId` before saving: inherit it from the intercepted/owning entity when applicable, otherwise resolve an authorized system/actor group. Never leave it undefined or take it unvalidated from context input.
