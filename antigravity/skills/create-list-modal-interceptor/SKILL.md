---
name: create-list-modal-interceptor
description: "Generates a custom class extending an abstract `SimpleListModalInterceptor` to dynamically modify list modal layouts, cell style rendering, database query filters, sidebar layouts, and sidebar load actions, and registers it."
---
<!-- Generated from src/skills/create-list-modal-interceptor/SKILL.md by tools/generate.py. Edit the source, not this file. -->
# Agent Skill: Create SimpleListModal Interceptor in Glyvio

This document defines a structured AI agent skill. Other AI coding agents or developers can load and execute this skill to generate and register an interceptor/listener to customize the behavior of an existing `SimpleListModal` view in another project.

---

## 🎯 Skill Metadata

- **Name**: `create_list_modal_interceptor`
- **Description**: Generates a custom class extending an abstract `SimpleListModalInterceptor` to dynamically modify list modal layouts, cell style rendering, database query filters, sidebar layouts, and sidebar load actions, and registers it.
...
1. Determine the temp file path: `antigravity/temp/<ModalName>_design.json` (e.g., `CustomerListModal_design.json`).
...
3. Create and register a temporary **SpyInterceptor** in the plugin:
   ...
   - Implement `SpyInterceptor` extending `<TargetBaseInterceptorClass>` to capture the `design` in `getDesign(state: <TargetStateClass>, design: glyvio_core.SimpleListModalDesign)`:
     ```typescript
     export class SpyInterceptor extends <TargetBaseInterceptorClass> {
       ...
       override getDesign(state: <TargetStateClass>, design: glyvio_core.SimpleListModalDesign): void {
...
4. Retrieve the design JSON using the browser inspection script:
   ...
   - Run the script `antigravity/scripts/chrome_inspector.js` via the available shell tool to connect to Chrome, automatically wait for `window.__finalDesign` to be populated, and save the retrieved JSON to `antigravity/temp/<ModalName>_design.json`.
     _Command:_ `node antigravity/scripts/chrome_inspector.js <ModalName>` (e.g., `node antigravity/scripts/chrome_inspector.js CustomerListModal`).
...
- Save the received JSON to `antigravity/temp/<ModalName>_cellDesign.json`.
...
1. **Target List Modal Name** (e.g., `CustomerListModal`): The name of the List Modal view class being intercepted.
2. **Target Route Class** (e.g., `CustomerListModalRoute`): The route class name of the target modal.
3. **Target Base Interceptor Class** (e.g., `CustomerListModalInterceptor`): The abstract base class of the interceptor defined by the target modal.
...
4. **Target State Class** (e.g., `CustomerListModalState`): The state interface of the target modal.
5. **Target Entity Name** (e.g., `Customer`): The name of the model in `glyvio_entity.*` associated with the modal.
6. **Listener Unique ID** (e.g., `custom_customer_list_modal_interceptor`): A unique identifier for the registered listener.
7. **Modifications Required**:
   - **Modal Design Modifications**: Customize headers, app bars, or sidebar filters.
   - **Cell Layout Modifications**: Customize rendering styles or color themes for specific row items.
...
glyvio_core.appInterceptorService.registerInterceptors([
  {
    interceptor: YourCustomListModalInterceptor,
...
/**
 * Custom Interceptor to dynamically augment the behavior of <TargetListModalName>.
 */
export class <InterceptorClassName> extends <TargetBaseInterceptorClass> {
  ...
  /**
   * Intercepts the SimpleListModalDesign configuration to customize visual layouts or App Bars.
  ...
  /**
   * Intercepts and alters cell layout designs representing rows inside the list.
   */
  override getDesignForCell(
    ...
    // 💡 Example: Change cell color theme conditionally
  }
  ...
  /**
   * Injects query restrictions on the main search filter.
   */
  ...
  override getLeftSidebar(
    state: <TargetStateClass>,
    design: glyvio_core.SimpleListSidebarModalDesign | undefined,
  ): glyvio_core.SimpleListSidebarModalDesign | undefined {
    return design;
  }
