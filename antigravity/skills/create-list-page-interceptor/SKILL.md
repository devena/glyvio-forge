---
name: create-list-page-interceptor
description: "Generates a custom class extending an abstract `SimpleListPageInterceptor` to dynamically modify list page design, cell layouts, and database query filters, and registers it."
---
<!-- Generated from src/skills/create-list-page-interceptor/SKILL.md by tools/generate.py. Edit the source, not this file. -->
# Agent Skill: Create List Page Interceptor in Glyvio

This document defines a structured AI agent skill. Other AI coding agents or developers can load and execute this skill to generate and register an interceptor/listener to customize the behavior of an existing `SimpleListPage` view in another project.

---

## 🎯 Skill Metadata

- **Name**: `create_list_page_interceptor`
- **Description**: Generates a custom class extending an abstract `SimpleListPageInterceptor` to dynamically modify list page design, cell layouts, and database query filters, and registers it.
...
1. Determine the temp file path: `antigravity/temp/<ListPageName>_design.json` (e.g., `ProductListPage_design.json`).
...
3. Create and register a temporary **SpyInterceptor** in the plugin:
   ...
   - Implement `SpyInterceptor` extending `<TargetBaseInterceptorClass>` to capture the `design` in `getDesign(state: <TargetStateClass>, design: glyvio_core.SimpleListPageDesign)`:
     ```typescript
     export class SpyInterceptor extends <TargetBaseInterceptorClass> {
       ...
       override getDesign(state: <TargetStateClass>, design: glyvio_core.SimpleListPageDesign): void {
...
4. Retrieve the design JSON using the browser inspection script:
   ...
   - Run the script `antigravity/scripts/chrome_inspector.js` via the available shell tool to connect to Chrome, automatically wait for `window.__finalDesign` to be populated, and save the retrieved JSON to `antigravity/temp/<ListPageName>_design.json`.
     _Command:_ `node antigravity/scripts/chrome_inspector.js <ListPageName>` (e.g., `node antigravity/scripts/chrome_inspector.js ProductListPage`).
...
5. Once the design JSON is collected and saved to `antigravity/temp/<ListPageName>_design.json`, remove the temporary `SpyInterceptor` and its registration from the codebase, and run `pnpm build` again to clean up the compiled distribution.
6. If Chrome debugging is not available or fails, fallback to asking the user to send the **current design JSON** of the page (obtained from `page.getDesignRaw(state)` or the console) in the next message, or to save it manually to `antigravity/temp/<ListPageName>_design.json`.
...
After completing the Step 0.1 analysis, **immediately write the findings to `antigravity/temp/<ListPageName>_analysis.md`** using the available file-editing tool. Include:
...
> "This task requires modifying the cell design. Please provide the current `LineCellDesign` JSON for a sample cell. You can obtain it by temporarily adding a `console.log(JSON.stringify(design))` inside a `getDesignForCell` override, then copying the output from the browser console."

- Save the received JSON to `antigravity/temp/<ListPageName>_cellDesign.json`.
...
1. **Target List Page Name** (e.g., `ProductListPage`): The name of the List Page view class being intercepted.
2. **Target Route Class** (e.g., `ProductListPageRoute`): The route class name of the target page.
3. **Target Base Interceptor Class** (e.g., `ProductListPageInterceptor`): The abstract base class of the interceptor defined by the target page.
...
4. **Target State Class** (e.g., `ProductListPageState`): The state interface of the target page.
5. **Target Entity Name** (e.g., `Product`): The name of the model in `glyvio_entity.*` associated with the page.
6. **Listener Unique ID** (e.g., `custom_product_list_page_interceptor`): A unique identifier for the registered listener.
7. **Modifications Required**:
   - **Page Design Modifications**: New filter fields, App Bar buttons, or columns to add.
   - **Cell Design Modifications**: Custom labels, chips, layouts, or text styling to inject into the cells.
   - **Query Modifications**: Additional database select logic or filters.
...
glyvio_core.appInterceptorService.registerInterceptors([
  {
    interceptor: YourCustomListPageInterceptor,
...
/**
 * Custom Interceptor to dynamically augment the behavior of <TargetListPageName>.
 */
export class <InterceptorClassName> extends <TargetBaseInterceptorClass> {
  ...
  /**
   * Intercepts the SimpleListPageDesign configuration to inject new filters or buttons.
  ...
  /**
   * Intercepts the cell design to inject custom labels, chips, layouts, or decorators.
   */
  override getDesignForCell(
    ...
    // 💡 Example: Adding a custom Chip to the cell:
    // design.child.children.push(new glyvio_core.ChipDesign({ ... }));
  }

  /**
   * Hook to modify or add criteria to the main query builder.
   */
  override populateQueryBuilder(
    state: <TargetStateClass>,
    queryBuilder: glyvio_core.QueryBuilder<glyvio_entity.<TargetEntityName>>,
  ): void {
    // 💡 Example: Add logic query filters:
  }
