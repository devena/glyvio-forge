...
## 🪆 Section-Native vs. Layout Counterpart (prefer the section-native class)

Several `glyvio_core` widgets ship in **two shapes**: a top-level `SectionDesign` subclass (e.g. `TableSectionDesign`, `ListSectionDesign`) that already carries its own `appBarDesign`/`columns`/`rows`/`cells`, and a nestable `LayoutDesign` counterpart (e.g. `TableLayoutDesign`) meant to be embedded **inside** another component's `child`/`childDesign` slot (a cell, a form field, a dashboard cell) — see `component_catalog.md`'s own entry for `TableLayoutDesign`: *"Contraparte layout (aninhável) do `TableSectionDesign`... dados tabulares **dentro** de outro container, não a página-tabela inteira."*

- **When building something that goes directly into `design.sectionsDesign`** (or any slot expressly typed `SectionDesign[]`), reach for the section-native class first (`TableSectionDesign`, not `TableLayoutDesign`). Wrapping the layout variant inside a bare `FormSectionDesign` just to place it in `sectionsDesign` is an **unnecessary extra nesting layer** — the section-native class already exposes the same `columns`/`rows`/`frozenColumnsCount`/`appBarDesign` API without a wrapper.
  - ❌ Wrong (unnecessary wrapper):
    ```typescript
    new glyvio_core.FormSectionDesign({
      appBarDesign: new glyvio_core.SimpleAppBarDesign({ title: 'History' }),
      childDesign: new glyvio_core.TableLayoutDesign({ columns, rows }),
    })
    ```
  - ✅ Correct (section-native, no wrapper):
    ```typescript
    new glyvio_core.TableSectionDesign({
      appBarDesign: new glyvio_core.SimpleAppBarDesign({ title: 'History' }),
      columns,
      rows,
    })
    ```
- **Only reach for the layout counterpart** when the table/list must be embedded *inside* a non-section slot — e.g. as a `FormLayoutFieldDesign.child`, inside a dashboard cell's `getDesignForCell`, or anywhere else the host slot's type is not `SectionDesign`.
- **How to tell which shape you have**: check the class's base in `@types`/source — if it extends `SectionDesign`, it belongs directly in a `sectionsDesign` array; if it extends `LayoutDesign`, it belongs inside a widget slot. Manufacturing a throwaway section wrapper whose only job is to host the layout variant defeats the purpose of the "Section Components — Top-Level Only" rule above.
- Before assuming a wrapper is needed, search `@types`/`dist/bundle.d.ts`/`component_catalog.md` for a `*SectionDesign` sibling of the layout class you were about to use, and prefer it when its constructor args already cover your needs.
- When delegating to coder subagents and when running any create/interceptor skill, **restate this rule** and make it part of the final validation audit.
...
## 📦 View Route Registration in `plugin/app/src/index.ts` (NON-NEGOTIABLE)

Every time a new **view** is created — page, modal, sidebar, or cart, from any `create-*` skill — its Route class **MUST be registered in `plugin/app/src/index.ts` in the same task**. A view that exists in code but is missing this is unreachable by the user (or by `pushModal`) and counts as a hard error, even though `tsc`/`eslint`/`webpack build` all stay clean — the compiler has no way to catch a missing route registration.

1. **Register the route** (URL matching / navigation) — **required for every view family, including modals, sidebars, and carts**, not pages only. Verified across `glyvio-plugin-sync`, `glyvio-plugin-crm`, and `glyvio-plugin-core`: every modal Route class (`XxxEditModalRoute`, `XxxEntityModalRoute`, etc.) is imported, `export *`-ed, and pushed into the same array as page routes:
   ```typescript
   glyvio_core.routerService.loadRoutes([MyPageRoute, MyModalRoute, MySidebarRoute]);
   ```
   Also add `export * from './path/to/my_view';` alongside the import, matching the sibling views already exported from the same `index.ts`.
2. **Instantiate the page** (registers the page handler in the framework) — **pages only**:
   ```typescript
   new MyPage();
   ```
3. **Add to the menu** (makes the page visible in the app sidebar) — **pages only**:
   ```typescript
...
**Modals, sidebars, and carts are opened programmatically** (`this.pushModal(new MyModalRoute({...}))`) and correctly skip steps 2 and 3 — but they still need step 1. Do not skip route registration for a modal just because it has no menu entry.

When delegating view creation to a coder subagent or running any `create-*` skill, **restate this rule** and include the route-registration wiring (step 1 always; steps 2–3 for pages only) in the final validation audit.
...
1. **Self-Correction Audit** (per `frontend_agent` rules):
   ...
   - "Did I wrap a *LayoutDesign* counterpart (e.g. `TableLayoutDesign`) inside a throwaway `FormSectionDesign` just to place it in `sectionsDesign`, when a section-native sibling (e.g. `TableSectionDesign`) already covers the same API?" → fix any violation; use the section-native class directly instead of a wrapper.
2. **Wiring Check**: Confirm **every newly created view** (page, modal, sidebar, cart) has its Route class registered via `routerService.loadRoutes` in `plugin/app/src/index.ts` — this is required for all view families, not pages only, and is a hard error if missing even though the build stays clean; for **pages** specifically, additionally confirm the page is instantiated (`new MyPage()`) and a menu entry is added (`FullMenuPage.fullMenuGroupAdd` or `fullMenuItemAdd`) — modals/sidebars/carts correctly skip these two; for **every newly created view**, confirm its `view` permission (`{ "type": "view", "subtype": "{entity}", "key": "{entity}_{view type}" }`) exists in `manifest.json` and is not duplicated — this is a hard error if missing; confirm interceptors are registered (`appInterceptorService.registerInterceptors`) with sensible `order`; confirm listener IDs are globally unique.
