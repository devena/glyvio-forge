# {{AGENT_DOC}} — plugin Glyvio

> Copie este arquivo para a **raiz do repositório do plugin** como `{{AGENT_DOC}}`.
> Ele é o único ponto que o Claude Code carrega automaticamente em toda sessão —
> sem ele, as regras arquiteturais em `{{RULES}}` **não entram no contexto**.
...
- `{{CATALOG}}` — índice rápido "o que parece → qual classe"
- `{{REFS_DIR}}/component_catalog_full.md` — props, hierarquia e exemplos
...
```sh
node {{SCRIPTS_DIR}}/validate_glyvio_rules.js
```
