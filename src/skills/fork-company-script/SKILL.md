...
| Private/internal JWT | Operator | `Authorization: Bearer <jwt>` for the fork call itself — must resolve to a real internal identity (private session), not an external-user or public token. The "one script per caller" tracking (see Gotchas) keys off this identity. **Two token shapes work, confirmed against a live backend:** a short-lived Cognito access token (expires in ~1h — will fail mid-multi-step-verification if you're not careful about timing), or a `gly_apk_`-prefixed Glyvio API key (confirmed **non-expiring** in practice — ask the operator for one of these instead when verification will span more than a few minutes, to avoid re-forking on expiry mid-sequence). Either way it's still the operator's to provide fresh — never reuse one cached from an earlier unrelated task. |
...
- **A `gly_apk_` API key is valid for the fork call and for private-flavored routes, but is explicitly rejected on external routes** — confirmed: hitting `/custom/external/...` with a `gly_apk_` token returns `403 {"permission":"invalid_api_key","type":"permission"}`. Don't reuse the same API key for a Step 4 verification request against an `external`-only controller; use whatever token type that specific endpoint actually expects (external-user JWT for `allowExternalUserAccess` controllers, etc.) — the API key is only for establishing the private identity needed to fork in the first place.
- **If the operator's "server base URL" is `app-beta.glyvio.com`, confirm it actually resolves to the API host before forking.** Confirmed in a separate context (browser-based live testing, see `test-plugin-browser`'s gotchas) that `app-beta.glyvio.com` can serve the SPA's HTML fallback instead of a real API response for custom-controller requests — the real API host observed there was `webapi-prod.glyvio.com`. If Step 4's verification request against `app-beta` comes back looking like an HTML page (or a route-not-found shape) rather than the expected JSON, don't assume the just-built code is broken — `curl -I` the base URL first to confirm it's actually the API, not the app's routing fallback.
