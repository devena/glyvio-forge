...
1. **`@@CATALOG@@` (and `@@REFERENCES@@/component_catalog_full.md`)** — the visual catalog. This is
...
3. The matching page skill(s) under `@@SKILLS@@/` (e.g. `@@SKILLS@@/create-list-page/SKILL.md`) for the final code blueprint.
...
Write the decomposition to `@@TEMP@@/<ScreenName>_visual_spec.json` as a component tree. Each node:
...
Then write a short human summary to `@@TEMP@@/<ScreenName>_visual_analysis.md`:
