...
(async () => {
  ...
  // Click on row 372 (first jeannie candidate, most recently seen as "Teste de funcionamento")
  await page.mouse.click(250, 372);
  await page.waitForTimeout(3000);
  let screens = await page.evaluate(() => window.__GLYVIO_AI__.describeCurrentScreens()).catch(() => []);
  let chatScreen = screens.find(s => s.nameObject === 'GlobalJeannieChatView');
  console.log('opened screen callbackId:', chatScreen?.callbackId);
  await page.screenshot({ path: shot('130_opened_which_session.png') });

  if (!chatScreen) { await browser.close(); return; }

  const dispatchPromise = page.evaluate(async (cb) => {
    try {
      const r = await window.__GLYVIO_AI__.dispatchAction(cb, '_SEND_MSG', {
        text: 'Conte de 1 a 5 devagar, uma linha por numero, sem pressa.',
  ...
  // rapid screenshots to catch mid-stream
  for (let i = 1; i <= 6; i++) {
    await page.waitForTimeout(1200);
    await page.screenshot({ path: shot(`131_stream_${i}.png`) });
  }
