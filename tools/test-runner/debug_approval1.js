...
(async () => {
  ...
  await page.mouse.click(250, 372); // any jeannie session
  ...
  // clear captured frames right before sending, so we only look at what this message triggers
  wsFrames.length = 0;
  ...
  // wait for the turn to run and (hopefully) request approval
