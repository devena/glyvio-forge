...
(async () => {
  ...
  console.log('setting override (no reloadPlugins call)...');
  ...
  console.log('reloading whole page...');
  await page.reload({ waitUntil: 'domcontentloaded', timeout: 45000 });
  await page.waitForTimeout(20000);
  console.log('url after page reload:', page.url());
  await page.screenshot({ path: shot('107_after_full_reload.png') });
  ...
  if (chatScreen) {
    ...
    console.log('session:', preState?.session?.id);
    ...
    for (let i = 0; i < 14; i++) {
      ...
      if (st?.pendingApproval) { approvalSeen = st.pendingApproval; console.log(`approval at t=${(i+1)*3}s:`, JSON.stringify(approvalSeen)); break; }
    }
    ...
    await page.screenshot({ path: shot('108_after_send.png') });
    fs.writeFileSync(shot('108_ws_frames.json'), JSON.stringify(wsFrames, null, 2));
  }
