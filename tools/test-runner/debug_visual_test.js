...
(async () => {
  ...
  if (!chatScreen) { await page.screenshot({ path: shot('120_no_screen.png') }); await browser.close(); return; }

  const dispatchResult = await page.evaluate(async (cb) => {
    try {
      const r = await window.__GLYVIO_AI__.dispatchAction(cb, '_SEND_MSG', {
        text: 'Liste 3 personas cadastradas e crie uma tarefa de teste chamada "Teste visual novo - pode ignorar" nesta sessao.',
  ...
  // take screenshots at intervals to catch the live bubble mid-stream, then after approval appears
  await page.waitForTimeout(6000);
  await page.screenshot({ path: shot('121_mid_turn.png') });
  await page.waitForTimeout(10000);
  await page.screenshot({ path: shot('122_after_more.png') });

  let approvalSeen = false;
  for (let i = 0; i < 10; i++) {
    await page.waitForTimeout(3000);
    const st = unwrap(await page.evaluate((cb) => window.__GLYVIO_AI__.getState(cb), chatScreen.callbackId));
    if (st?.pendingApproval) { approvalSeen = true; console.log('approval:', JSON.stringify(st.pendingApproval)); break; }
  }
  await page.screenshot({ path: shot('123_final_with_approval.png') });
  console.log('approval seen:', approvalSeen);
