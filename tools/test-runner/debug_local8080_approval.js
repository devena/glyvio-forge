...
(async () => {
  ...
  page.on('websocket', ws => {
    ws.on('framereceived', f => wsFrames.push({ ts: Date.now(), payload: f.payload?.toString?.() ?? String(f.payload) }));
  });
  ...
  if (!chatScreen) { await page.screenshot({ path: shot('102_no_chat_screen.png') }); await browser.close(); return; }
  ...
  // poll for pendingApproval every 3s for up to 40s
  let approvalSeen = null;
  for (let i = 0; i < 14; i++) {
    await page.waitForTimeout(3000);
    const st = unwrap(await page.evaluate((cb) => window.__GLYVIO_AI__.getState(cb), chatScreen.callbackId));
    if (st?.pendingApproval) { approvalSeen = st.pendingApproval; console.log(`approval seen at t=${(i+1)*3}s:`, JSON.stringify(approvalSeen)); break; }
    if (st?.pendingQuestions) { console.log(`question seen at t=${(i+1)*3}s:`, JSON.stringify(st.pendingQuestions)); }
  }
  if (!approvalSeen) console.log('no approval seen within poll window');

  const finalState = unwrap(await page.evaluate((cb) => window.__GLYVIO_AI__.getState(cb), chatScreen.callbackId));
  console.log('final pendingApproval:', JSON.stringify(finalState?.pendingApproval));
  console.log('final liveToolTrace:', JSON.stringify(finalState?.liveToolTrace));
  console.log('final liveTurnActive:', finalState?.liveTurnActive);

  await page.screenshot({ path: shot('103_local8080_after_send.png') });
  fs.writeFileSync(shot('103_ws_frames.json'), JSON.stringify(wsFrames, null, 2));
