...
(async () => {
  ...
  for (const y of candidateYs) {
    ...
    console.log(`y=${y} -> session.id = ${state?.session?.id}`);
    if (state?.session?.id === TARGET_ID) {
      found = chatScreen;
      break;
    }
    // go back to list for next attempt
    await page.mouse.click(1291, 33);
    await page.waitForTimeout(3000);
  }

  if (!found) {
    console.log('TARGET SESSION NOT FOUND in visible rows');
    await browser.close();
    return;
  }

  console.log('FOUND target session, callbackId:', found.callbackId);
  await page.screenshot({ path: shot('60_target_before_refetch.png') });
  ...
  console.log('manual fetch result:', JSON.stringify(r1).slice(0, 500));

  await page.waitForTimeout(3000);
  await page.screenshot({ path: shot('61_target_after_refetch.png') });

  const errors = await page.evaluate((cb) => window.__GLYVIO_AI__.getErrors({ callbackId: cb, limit: 20 }), found.callbackId);
