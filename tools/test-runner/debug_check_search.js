...
(async () => {
  ...
  // click search field and type the code
  await page.mouse.click(260, 110);
  await page.waitForTimeout(500);
  await page.keyboard.type('505792', { delay: 50 });
  await page.waitForTimeout(2500);
  await page.screenshot({ path: shot('115_search_505792.png') });
  ...
  if (chatScreen) {
    ...
    console.log('session.id:', st?.session?.id, 'sessionIdentifier:', st?.session?.sessionIdentifier);
    console.log('pendingApproval:', JSON.stringify(st?.pendingApproval));
    console.log('pendingQuestions:', JSON.stringify(st?.pendingQuestions));
  }
  await page.screenshot({ path: shot('116_search_opened.png') });
