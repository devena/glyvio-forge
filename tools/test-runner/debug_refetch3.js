...
(async () => {
  ...
  const dispatchResult = await page.evaluate(async (cb) => {
    try {
      const r = await window.__GLYVIO_AI__.dispatchAction(cb, '_FETCH_MESSAGES', { limit: 30, offset: 0 }, { unsafeMode: true });
      return { ok: true, result: r };
    } catch (e) { return { ok: false, error: e.message }; }
  }, chatScreen.callbackId);
  fs.writeFileSync(shot('51_dispatch_result.json'), JSON.stringify(dispatchResult, null, 2));
  console.log('manual _FETCH_MESSAGES (unsafeMode):', JSON.stringify(dispatchResult).slice(0, 3000));

  await page.waitForTimeout(2000);
  await page.screenshot({ path: shot('51_after_unsafe_refetch.png') });
