...
(async () => {
  ...
  const result = await page.evaluate(async () => {
    const allAgents = await window.queryForUser(
      "SELECT id, name, plugin_id, deleted FROM chat_agent ORDER BY created_at DESC LIMIT 30",
      []
    );
    return { allAgents };
  }).catch(e => ({ error: e.message }));
  fs.writeFileSync(
    '/tmp/claude-1000/-opt-data-glyvio-services/7b3f0d55-c695-4b5f-90c8-ab63a57cb02d/scratchpad/26_all_agents.json',
