...
(async () => {
  ...
  const jsErrors = [];
  page.on('pageerror', err => jsErrors.push({ type: 'pageerror', text: err.message, stack: err.stack }));
  page.on('console', msg => {
    if (msg.type() === 'error' || msg.type() === 'warning') jsErrors.push({ type: 'console-' + msg.type(), text: msg.text() });
  });
  ...
  jsErrors.length = 0; // reset right before opening the target session
  ...
  for (const y of candidateYs) {
    ...
    await page.mouse.click(1291, 33);
    await page.waitForTimeout(3000);
    jsErrors.length = 0; // reset for next attempt
  }

  console.log('found target:', !!found);
  fs.writeFileSync(shot('80_full_js_errors.json'), JSON.stringify(jsErrors, null, 2));
  console.log('errors captured around target session open:', jsErrors.length);
