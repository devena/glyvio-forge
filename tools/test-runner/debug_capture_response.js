...
(async () => {
  ...
  page.on('response', async (res) => {
    ...
    if (req.url().includes('/query-for-user') && req.method() === 'POST') {
      try {
        ...
        if (reqBody?.query?.includes('FROM chat_message')) {
          const status = res.status();
          let respBody = null;
          try { respBody = await res.json(); } catch (e) { respBody = 'parse-error: ' + e.message; }
          exchanges.push({ status, reqBody, respBody });
        }
      } catch (e) {}
    }
  });
  ...
  await page.waitForTimeout(2000);
  fs.writeFileSync(shot('71_response_capture.json'), JSON.stringify(exchanges, null, 2));
  for (const ex of exchanges) {
    console.log('status:', ex.status, 'response rows:', Array.isArray(ex.respBody) ? ex.respBody.length : ex.respBody);
  }
