...
1. **`codex/component_catalog.md` (and `codex/references/component_catalog_full.md`)** — the visual catalog. This is
...
3. The matching page skill(s) under `codex/skills/` (e.g. `codex/skills/create-list-page/SKILL.md`) for the final code blueprint.
...
Write the decomposition to `codex/temp/<ScreenName>_visual_spec.json` as a component tree. Each node:
...
Then write a short human summary to `codex/temp/<ScreenName>_visual_analysis.md`:
